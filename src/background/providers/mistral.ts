...
export class MistralProvider implements AIProvider {
  ...
  async checkGrammar(text: string, language: string): Promise<GrammarError[]> {
    const raw = await callMistral(buildGrammarPrompt(text, language), this.apiKey, this.model)
  ...
  async rewrite(text: string, selection: string | undefined, language: string, tone?: TonePreset): Promise<string> {
    ...
    return callMistral(prompt, this.apiKey, this.model)
  }

  async translate(text: string, targetLanguage: string): Promise<string> {
    return callMistral(buildTranslatePrompt(text, targetLanguage), this.apiKey, this.model)
  }
}
